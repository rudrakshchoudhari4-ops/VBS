...
public class TransactionController {
...

    {
        User user = userRepo.findById(obj.getId())
                .orElseThrow(()->new RuntimeException("Id not found"));
        double sbalance = user.getBalance() - obj.getAmount();
        if(sbalance<0)
        {
            return "Balance Insufficient";
        }
        user.setBalance(sbalance);
        userRepo.save(user);

        Transaction t = new Transaction();
        t.setAmount(obj.getAmount());
        t.setCurrBalance(sbalance);
    ...
    public String transfer(@RequestBody TransferDto obj){
        ...
        t1.setDescription("Rs"+obj.getAmount()+" Send to "+obj.getUsername());
        t1.setUserId(sender.getId());

        t2.setAmount(obj.getAmount());
        t2.setCurrBalance(rbalance);
        t2.setDescription("Rs"+obj.getAmount() +" Received From "+ obj.getId());
